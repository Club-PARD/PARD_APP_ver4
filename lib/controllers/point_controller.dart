...
class PointController extends GetxController {
  ...
  Future<void> getCurrentUserPartRank() async {
    try {
      ...
      List<MapEntry<UserModel, double>> samePartEntries =
          sortedEntries
              .where(
                (entry) =>
                    entry.key.part == _userController.userInfo.value!.part,
              )
              .toList();
  ...
  Future<void> attendQR(UserModel user, int attendPoint) async {
    try {
      String? pid = user.pid;
      DocumentReference pointsRef = FirebaseFirestore.instance
          .collection('points')
          .doc(pid); //user's pid
