...
import 'package:pard_app/models/point_model/user_point.dart';
import 'dart:convert';
...
class SpringPointController extends GetxController {
  ...
  final SpringUserController springUserController =
      Get.find<SpringUserController>();
  ...
  Future<void> fetchTop3Rank() async {
    try {
      ...
      final uri = Uri.parse(
        '${dotenv.env['SERVER_URL']}/v1/rank/top3?generation=$generation',
      );
      print(uri);
      final response = await http.get(
        uri,
      ...
      if (response.statusCode == 200) {
        ...
        List<RankPointModel> top3Rank =
            jsonResponse.map((data) => RankPointModel.fromJson(data)).toList();
  ...
  Future<void> fetchTotalRank() async {
    try {
      ...
      String? generation = springUserController.userInfo.value?.generation;
      final uri = Uri.parse(
        '${dotenv.env['SERVER_URL']}/v1/rank/total?generation=$generation',
      );
      final response = await http.get(
        uri,
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        List<RankingResponseDTO> rankingListResponse =
            jsonResponse
  ...
  Future<void> fetchReasonBonus() async {
    try {
      ...
      if (response.statusCode == 200) {
        List<dynamic> jsonResponse = json.decode(response.body);
        List<ReasonBonus> reasonBonusListResponse =
            jsonResponse.map((data) => ReasonBonus.fromJson(data)).toList();
